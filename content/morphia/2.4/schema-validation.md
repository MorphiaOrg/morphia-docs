...
Morphia provides annotations that allow developers to define document validations for a collection alongside the other mapping data on an entity's source. [Schema validation](https://docs.mongodb.com/manual/core/schema-validation/) provides the capability to perform schema validation during updates and insertions.
...
apply mapped document validations when `morphia.apply-document-validations` is set to true in the [configuration file](../configuration/).
...
This string value is the query that will be used to match against any new or updated documents.
