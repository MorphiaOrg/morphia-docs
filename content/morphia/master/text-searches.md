...
[test](https://github.com/MorphiaOrg/morphia/tree/master/core/src/test/java/dev/morphia/query/TestTextSearching.java) does:
...
List<Greeting> good = datastore.find(Greeting.class)
                               .filter(text("good"))
...
```java
datastore.find(Greeting.class, new FindOptions()
                       .sort(ascending("_id")))
         .filter(text("good")
                     .language("english"))
         .iterator()
